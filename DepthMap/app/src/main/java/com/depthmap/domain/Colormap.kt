...
fun applyInfernoColormap(depthArray: FloatArray, width: Int, height: Int): Bitmap {
    ...
    if (range > 1e-6f) {
        for (i in depthArray.indices) {
            val idx = ((depthArray[i] - minVal) / range * 255.0).toInt().coerceIn(0, 255)
            pixels[i] = Color.argb(255, INFERNO_RED[idx], INFERNO_GREEN[idx], INFERNO_BLUE[idx])
        }
    }
